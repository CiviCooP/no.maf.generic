 ...
 * @license AGPL-3.0
 */
class CRM_Generic_ConfigItems_CustomDataQuery {
  ...
  /**
   * CRM_Generic_ConfigItems_CustomDataQuery constructor.
   *
   * @param array $params
   */
  ...
  private function validateSourceParams() {
    foreach ($this->_requiredParams as $required) {
      if (!isset($this->_sourceParams[$required])) {
        throw new Exception(ts('Missing mandatory param ').$required.ts(' in CRM_Generic_ConfigItems_CustomDataQuery'));
      }
    }
    if (empty($this->_sourceParams['custom_fields']) || !is_array($this->_sourceParams['custom_fields'])) {
      throw new Exception(ts('Could not find any data to build a custom data query for in CRM_Generic_ConfigItems_CustomDataQuery'));
    }
    if (empty($this->_sourceParams['table_name'])) {
      throw new Exception(ts('Empty table_name in params, could not build query for custom data in CRM_Generic_ConfigItems_CustomDataQuery'));
    }
    if (empty($this->_sourceParams['entity_id'])) {
      throw new Exception(ts('Empty entity_id in params, could not build query for custom data in CRM_Generic_ConfigItems_CustomDataQuery'));
    }
    if (empty($this->_sourceParams['query_action'])) {
      throw new Exception(ts('Empty query_action in params, could not build query for custom data in CRM_Generic_ConfigItems_CustomDataQuery'));
    }
    ...
    if (!in_array($this->_sourceParams['query_action'], $this->_validQueryActions)) {
      throw new Exception(ts('Invalid query_action '.$this->_sourceParams['query_action']
        .' in params, could not build query for custom data in CRM_Generic_ConfigItems_CustomDataQuery'));
    }
  }
}